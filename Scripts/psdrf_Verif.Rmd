...
file <- tk_choose.files(caption = "Choix d'une réserve", filters=matrix(c(".xls",".xlsx"),1,2, byrow = T))
setwd(dirname(file))
...
TempTarifs            <- subset(Tarifs, NumDisp==TempPlacettes$NumDisp[1])
```
...
cat(paste("Nombre de BMSsup30 :",dim(TempBMSsup30)[1]))
...
```{r, echo=F, comment=NA} 
if (sum(duplicated(TempTarifs[,1:2])) >0) {
